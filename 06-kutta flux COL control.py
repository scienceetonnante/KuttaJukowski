...
U0 = 1              # Velocity
a = 1               # Radius
b1 = 0.05            # Joukowski shift
b2 = 0.2
c = 0.9             # Joukowski deformation
alpha = 10/180*pi   # Angle of the flow

gamma = -4*pi*U0*(a+b1)*np.sin(alpha) - 4*pi*U0*b2*np.cos(alpha)   # Circulation to get Kutta condition
theta = alpha                          # Global rotation so that the flow is horizontal
z0 = -b1 + 1j*b2                         # Shift of circle 
...
#OUTPUT = True
#name = "03-kutta flux NB 1part"
#NFRAMES = 450
#FPS = 30
#STEP = 30
#DT = 0.001
#
#particles = [np.r_[seeds[0,k],seeds[1,k]] for k in [28]]
##
...
#plt.savefig("06-kutta flux COL.png")